...
export class UomComponent implements OnInit {
  ...
  gridData: any[] = [];
  sort: SortDescriptor[] = [{ field: 'UOM_Order', dir: 'asc' }];
  ...
  load() {
    ...
    this.http.get<any>('/api/refdata/uom').pipe(map((r: any) => r.data)).subscribe({
  ...
  filter() {
    ...
    this.gridData = orderBy(
      q ? this.allItems().filter(r =>
        r.UOM_UnitDescShort?.toLowerCase().includes(q) ||
        r.UOM_UnitDesc?.toLowerCase().includes(q)
      ) : this.allItems(),
      this.sort
    );
    this.skip = 0;
  ...
  openNew() {
    ...
    this.form.reset({ shortDesc: '', fullDesc: '', sortOrder: null, isActive: true });
    this.form.enable();
  ...
  openEdit(item: any) {
    ...
    this.viewMode.set(false);
    this.form.reset({ shortDesc: item.UOM_UnitDescShort, fullDesc: item.UOM_UnitDesc, sortOrder: item.UOM_Order, isActive: item.UOM_IsActive });
    this.form.enable();
    this.showDialog.set(true);
  }
  openView(item: any) {
  ...
  deactivate(item: any) {
    ...
    this.http.put<any>('/api/refdata/deactivate', { entity: 'uom', id: item.UOM_ID }).subscribe({
      next: () => this.load(),
  ...
  save() {
    ...
    const id = this.editItem()?.UOM_ID;
    const req = id
      ? this.http.put<any>(`/api/refdata/uom/${id}`, { shortDesc: v.shortDesc, fullDesc: v.fullDesc, sortOrder: v.sortOrder, isActive: v.isActive })
      : this.http.post<any>('/api/refdata/uom', { shortDesc: v.shortDesc, fullDesc: v.fullDesc, sortOrder: v.sortOrder });
    req.subscribe({
