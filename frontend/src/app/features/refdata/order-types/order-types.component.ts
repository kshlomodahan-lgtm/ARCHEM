...
@Component({
  ...
  styleUrl: './order-types.component.scss',
})
export class OrderTypesComponent implements OnInit {
  ...
  gridData: any[] = [];
  sort: SortDescriptor[] = [{ field: 'OT_ID', dir: 'asc' }];
  ...
  load() {
    ...
    this.http.get<any>('/api/refdata/order-types').pipe(map((r: any) => r.data)).subscribe({
  ...
  filter() {
    ...
    this.gridData = orderBy(
      q ? this.allItems().filter(r =>
        r.OT_OrderTypeDesc?.toLowerCase().includes(q) ||
        r.OT_OrderTypeDescEng?.toLowerCase().includes(q)
      ) : this.allItems(),
      this.sort
    );
    this.skip = 0;
  ...
  openNew() {
    ...
    this.form.reset({ descHE: '', descEN: '', explanation: '', isActive: true });
    this.form.enable();
  ...
  openEdit(item: any) {
    ...
    this.viewMode.set(false);
    this.form.reset({ descHE: item.OT_OrderTypeDesc, descEN: item.OT_OrderTypeDescEng, explanation: item.OT_ExtendedExplanation, isActive: item.OT_IsActive });
    this.form.enable();
    this.showDialog.set(true);
  }
  openView(item: any) {
  ...
  deactivate(item: any) {
    ...
    this.http.put<any>('/api/refdata/deactivate', { entity: 'order-types', id: item.OT_ID }).subscribe({
      next: () => this.load(),
  ...
  save() {
    ...
    const id = this.editItem()?.OT_ID;
    const req = id
      ? this.http.put<any>(`/api/refdata/order-types/${id}`, { descHE: v.descHE, descEN: v.descEN, explanation: v.explanation, isActive: v.isActive })
      : this.http.post<any>('/api/refdata/order-types', { descHE: v.descHE, descEN: v.descEN, explanation: v.explanation });
    req.subscribe({
