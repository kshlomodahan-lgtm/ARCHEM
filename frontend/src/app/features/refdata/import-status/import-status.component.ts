...
export class ImportStatusComponent implements OnInit {
  ...
  gridData: any[] = [];
  sort: SortDescriptor[] = [{ field: 'IPS_ID', dir: 'asc' }];
  skip = 0; pageSize = 50; searchTerm = '';

  form = this.fb.group({
    status: ['', Validators.required],
    totalData: [0 as number | null],
    isActive: [true],
  });
  ...
  load() {
    ...
    this.http.get<any>('/api/refdata/import-status').pipe(map((r: any) => r.data)).subscribe({
  ...
  filter() {
    ...
    this.gridData = orderBy(
      q ? this.allItems().filter(r => r.IPS_Status?.toLowerCase().includes(q)) : this.allItems(),
      this.sort
    );
    this.skip = 0;
  }
  ...
  openNew() {
    ...
    this.form.reset({ status: '', totalData: 0, isActive: true });
    this.form.enable();
  ...
  openEdit(item: any) {
    ...
    this.viewMode.set(false);
    this.form.reset({ status: item.IPS_Status, totalData: item.IPS_TotalDataToFillIn, isActive: item.IPS_isActive });
    this.form.enable();
    this.showDialog.set(true);
  }
  openView(item: any) {
  ...
  deactivate(item: any) {
    ...
    this.http.put<any>('/api/refdata/deactivate', { entity: 'import-status', id: item.IPS_ID }).subscribe({
      next: () => this.load(),
  ...
  save() {
    ...
    const id = this.editItem()?.IPS_ID;
    const req = id
      ? this.http.put<any>(`/api/refdata/import-status/${id}`, { status: v.status, totalData: v.totalData, isActive: v.isActive })
      : this.http.post<any>('/api/refdata/import-status', { status: v.status, totalData: v.totalData });
    req.subscribe({
