...
export class ItemCategoriesComponent implements OnInit {
  ...
  gridData: any[] = [];
  sort: SortDescriptor[] = [{ field: 'CategoryName', dir: 'asc' }];
  ...
  load() {
    ...
    this.http.get<any>('/api/refdata/item-categories').pipe(map((r: any) => r.data)).subscribe({
  ...
  openEdit(item: any) {
    ...
    this.form.reset({ categoryName: item.CategoryName, levelName: item.LevelName, isActive: item.IsActive });
    this.form.enable();
  ...
  deactivate(item: any) {
    ...
    this.http.put<any>('/api/refdata/deactivate', { entity: 'item-categories', id: item.CategoryID }).subscribe({
      next: () => this.load(),
  ...
  save() {
    ...
    const id = this.editItem()?.CategoryID;
    this.http.put<any>(`/api/refdata/item-categories/${id}`, { categoryName: v.categoryName, levelName: v.levelName, isActive: v.isActive }).subscribe({
