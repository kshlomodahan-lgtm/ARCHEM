...
export class CitiesComponent implements OnInit {
  ...
  load() {
    ...
    this.http.get<any>('/api/refdata/cities').pipe(map((r: any) => r.data)).subscribe({
  ...
  filter() {
    ...
    this.gridData = orderBy(
      q ? this.allItems().filter(r =>
        ...
        r.CountryNameHE?.toLowerCase().includes(q)
      ) : this.allItems(),
      this.sort
    );
    this.skip = 0;
  ...
  openNew() {
    ...
    this.form.reset({ countryId: null, nameHE: '', nameEN: '', postalCode: '', isActive: true });
    this.form.enable();
  ...
  openEdit(item: any) {
    ...
    this.viewMode.set(false);
    this.form.reset({ countryId: item.CountryID, nameHE: item.NameHE, nameEN: item.NameEN, postalCode: item.PostalCode, isActive: item.IsActive });
    this.form.enable();
    this.showDialog.set(true);
  }
  openView(item: any) {
  ...
  deactivate(item: any) {
    ...
    this.http.put<any>('/api/refdata/deactivate', { entity: 'cities', id: item.CityID }).subscribe({
      next: () => this.load(),
  ...
  save() {
    ...
    const id = this.editItem()?.CityID;
    const req = id
      ? this.http.put<any>(`/api/refdata/cities/${id}`, { countryId: v.countryId, nameHE: v.nameHE, nameEN: v.nameEN, postalCode: v.postalCode, isActive: v.isActive })
      : this.http.post<any>('/api/refdata/cities', { countryId: v.countryId, nameHE: v.nameHE, nameEN: v.nameEN, postalCode: v.postalCode });
    req.subscribe({
